...
onMounted(() => {
    ...
    // Populate weekData and set selectedGender for restaurant timings
    ...
    admin_app_details(); // Get restaurant details
    get_category();
    store.fetchContacts();
});
...
async function delete_time(item) {
    const formData = new FormData();
    formData.append('time_id', item);
    try {
        const response = await axios.post('/api/v_super/delete_time/', formData);
        if (response.status >= 200 && response.status < 300) {
            console.log('Response data:', response.data);
            admin_app_details();
        } else {
            console.error(`Unexpected status code: ${response.status}`);
            alert(`Failed to delete item. Status code: ${response.status}`);
        }
    } catch (error) {
...
const admin_app_details = async () => {
    ...
    if (response.data.status === 'done') {
        new_loader.value = false;
        const updatedData = response.data.v_res_details.map((item, index) => {
            ...
            const timingsValues = Array.isArray(item.timings) ? item.timings.map((c) => c) : [];
            console.log('check the data ---- in timings value ---', timingsValues);

            // Loop over the restaurant timings and assign them to selectedGender

            return {
                show_time:item.timings,
                id: item.id,
                serial_no: `#${index + 1}`,
                tagModel: topicsValues,
                avatar: item.image,
                ...
                longitude: item.longitude,
                restaurantType: 'done',
                monStartTimeMdl: timingsValues.map(t => t.opening_hours),
                monEndTimeMdl: timingsValues.map(t => t.closing_hours),
            ...
            <div class="d-flex flex-wrap gap-2 justify-md-end">
                ...
                <div>
                    <v-dialog v-model="dialog" max-width="800">
                        <template v-slot:activator="{ props }">
                            <v-btn color="primary" v-bind="props" flat> <v-icon class="mr-2">mdi-plus</v-icon> Add Restaurant </v-btn>
                        </template>
                        <v-card>
                            <v-card-title class="pa-4 bg-primary">
                            ...
                            <v-card-text>
                                <v-form ref="form" v-model="valid" lazy-validation @submit.prevent="submit_res_Form">
                                    <v-row>
                                       ...
                                       <!-- Loop through the show_time array -->
                                       <!-- <div v-for="(time, index) in editedItem.show_time" :key="index">
                                            <div>{{ time.day }}: {{ time.opening_hours }} to {{ time.closing_hours }}</div>
                                        </div> -->
                                        ...
                                        <v-col cols="12">
                                            <div v-if="editedItem.show_time" class="bg-lightsecondary pa-4 removeArrow">
                                                <h3 class="text-h6 mb-4">Opening Timings</h3>
                                                <v-row class="ma-n1">
                                                    <v-col
                                                        cols="12"
                                                        md="6"
                                                        lg="4"
                                                        class="px-1 py-0"
                                                       v-for="(time, index) in editedItem.show_time"
                                                        :key="index"
                                                    >

                                                    <v-checkbox
                                                        :label="time.day"
                                                        v-model="time.is_active"
                                                        color="primary"
                                                        hide-details
                                                        @change="delete_time(time.id)"
                                                    >
                                                    </v-checkbox>
                                        ...
                                        <v-col cols="12">
                                            <div class="bg-lightsecondary pa-4 ">
                                                <h3 class="text-h6 mb-4">Add Restaurant Weekdays Timing</h3>
                                                <v-row class="ma-n1">
                                                    <v-col
                                                        cols="12"
                                                        md="6"
...
.removeArrow .v-field__append-inner {
    display: none;
}
</style>