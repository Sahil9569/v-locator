...
import complete_order from '@/_mockApis/apps/manage-order/complete_order';
...
const desserts = ref(complete_order);
    ...
    try {
        ...
        if (response.data) {
            ...
            const updatedData = response.data.data.map((item:any, index:any) => {
                // const amountInfo = item.items.map(item => {
                //     const itemTotal = item.quantity * item.item.dish_price;
                //     return `${itemTotal}`;
                // }).join(', ');
                return {
                    SNo: `#${index + 1}`,
